...
function deviceIsTablet() {
    ...
    {
        // check to see if the device is an Android
        if (userAgent.search("android") >= 0)
        {
            // is Andriod so check to see if the user agent contains "mobile" which
            // does not exist for an Android tablet
...
{
    var isEmailValid;
    var regexValidEmail = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;  // checks for valid email
...
/// -----------------------------------------------------------------------
/// Provides the ability to navigate to the passed url.
...
/// ```
/// // create new event:
/// var myEvent = initCustomEvent('NAME_OF_CUSTOM_EVENT');
/// // dispatch event:
...
function initCustomEvent(customEventName) {
    ...
    // ie11 (& below) is unable to use the `Event()` constructor, so we use a polyfill (n.b. dispatching events is the same in both cases):
    try
    {
        event = new Event(customEventName);
    }
    // ie utilizes the depricated `initEvent()` function:
    catch (error)
    {
